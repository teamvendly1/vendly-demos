 ...
 *     };
 *   </script>
 *   <script src="https://vendly-assistant.vercel.app/siteassistant-v2.js" defer></script>
 *
...
(function () {
  ...
  function injectCss(accent, theme) {
    ...
    var css =
      ...
      "font-size:14px;font-family:inherit;color:var(--sa-ink);background:var(--sa-field)}" +
      ...
      ".sa-panel{left:0;right:0;bottom:0;width:100%;max-width:100%;height:88vh;max-height:88vh;" +
  ...
  SiteAssistant.prototype.boot = function () {
    var self = this;
    var cfg = this.cfg;
    // ---- page-view beacon, INDEPENDENT of the chat kill switch --------------
    // ANALYTICS AND CHAT ARE TWO SEPARATE PRODUCTS and a client may buy one and
    // decline the other. Palmetto is exactly that case: the chat widget is
    // declined, the data / analytics / weekly rundown are wanted and always were
    // (_tmp/PALMETTO-CONCIERGE-TRUTH-FOREVER.txt).
    //
    // This call used to live INSIDE render(), which meant the `active:false`
    // branch below returned before it and the beacon never fired at all. The
    // storefront carried the script tag, loaded the bundle, and recorded exactly
    // nothing - a state indistinguishable from "installed and working" unless
    // you read the counters. A TAG PRESENT IS NOT DATA FLOWING.
    //
    // The split that keeps this fixed: the SERVER decides whether to STORE an
    // event (/api/track drops unknown tenants, and inactive tenants that do not
    // have backendMonitoring), the CLIENT decides only whether to DRAW UI. So a
    // chat preference can never silently switch off a client's analytics again,
    // and turning analytics off stays a deliberate server-side decision.
    var beaconed = false;
    var beaconView = function () {
      if (beaconed) return;
      beaconed = true;
      // One page view per load; path buckets the top-pages report.
      var path = "/";
    ...
    // ---- HARD chat suppression, with NO network call ------------------------
    // The /api/status check below deliberately FAILS OPEN so a transient blip
    // never hides a paying client's widget. That is the right default when the
    // failure mode is a MISSING feature. It is exactly the wrong default when
    // the failure mode is an UNWANTED one: for a client who declined the chat,
    // one unreachable request draws the thing he refused onto his storefront.
    //
    // Palmetto is that client. His storefront still carries this bundle, so his
    // chat-off state was resting entirely on a network round trip succeeding.
    // It is now enforced here, before any fetch, so no outage, no DNS blip and
    // no cached-bundle-plus-dead-API window can put a launcher on his site.
    //
    // The beacon above has ALREADY fired at this point, which is the whole
    // point: he keeps every number, and the chat surface never exists.
    //
    // Two independent switches, either one is enough:
    //   cfg.chatEnabled === false   set in the page's SiteAssistantConfig
    //   DECLINED[tenantId]          compiled in, for a theme we do not control
    // hasOwnProperty, not a bare lookup, so a tenant id like "constructor"
    // cannot inherit a truthy value off Object.prototype.
    var DECLINED = { palmetto: true };
    var tid = cfg && typeof cfg.tenantId === "string" ? cfg.tenantId.toLowerCase() : "";
    if (
      (cfg && cfg.chatEnabled === false) ||
      (tid && Object.prototype.hasOwnProperty.call(DECLINED, tid))
    ) {
      return;
    }
  ...
  // Keep the mobile bottom sheet pinned to the VISIBLE viewport so the on-screen
  // keyboard never covers the input row and the panel cannot shift off screen.
  SiteAssistant.prototype.syncViewport = function () {
    var vv = window.visualViewport;
    var p = this.panel;
    if (!p) return;
    if (!vv || window.innerWidth > 480) {
      p.style.top = ""; p.style.height = ""; p.style.bottom = "";
      return;
    }
    p.style.top = Math.max(0, vv.offsetTop) + "px";
    p.style.bottom = "auto";
  ...
  SiteAssistant.prototype.bindViewport = function () {
    ...
    if (vv) {
      ...
      // iOS reports the resized viewport late; re-pin a few times after focus.
      this._focusHandler = function () {
        [0, 150, 350, 600].forEach(function (t) { setTimeout(function () { if (self.open) self.syncViewport(); }, t); });
      };
  ...
  SiteAssistant.prototype.unbindViewport = function () {
    ...
    if (this._focusHandler) { try { this.input.removeEventListener("focus", this._focusHandler); } catch (e) {} }
    this._vvHandler = null; this._focusHandler = null;
    if (this._bodyPrev) {
      var b = document.body;
    ...
    this.panel.style.top = ""; this.panel.style.height = ""; this.panel.style.bottom = "";
  };
